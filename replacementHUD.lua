    ...
    for i = 1, globals.maxplayers() do
        if (entity.get_classname(i) == "CCSPlayer") then
            ...
            if (observer ~= nil) then       
                ...
                local observerSteamID = GameStateAPI.GetPlayerXuidStringFromEntIndex(i)
...
function player_chat(e)
    if ui.get(controls[1]) then
        if e.entity == nil then return end
        local steamid = GameStateAPI.GetPlayerXuidStringFromEntIndex(e.entity)
...
client.set_event_callback('player_chat', player_chat)
...
function on_aim_hit(e)
    if ui.get(controls[1]) then
        local damage = e.damage
        local hitbox = hitGroups[e.hitgroup]
        if (hitbox == nil or hitbox == "") then 
            hitbox = "generic"
        end
